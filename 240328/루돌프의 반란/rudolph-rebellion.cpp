...
void interaction(int santa_num, int curX, int curY, int dirX, int dirY) { //santa_num이 (curX,curY)로 오고, (curX,curY)에 있던 산타는 dirX, dirY 방향으로 한 칸씩 밀려남
    ...
    if (curX + dirX<1|| curX + dirX>N|| curY + dirY<1|| curY + dirY>N) { //밀려날곳이 격자판 넘어가면
        santa_status[pushed_santa_num].first.first = 2;
    }
    else if (santa_matrix[curX + dirX][curY + dirY]) {
...
void crashByRudolph(int Mr, int Mc) { //루돌프가 움직여서 충돌난 경우
    ...
    if (Rr + C * Mr<1 || Rr + C * Mr>N || Rc + C * Mc<1 || Rc + C * Mc>N) {
        santa_status[santa_num].first.first = 2; //탈락
    }
    else if (santa_matrix[Rr + C * Mr][Rc + C * Mc]) { //밀려날 곳:[Rr+C*Mr][Rc+C*Mc]에 다른 산타가 있다면 상호작용
...
void crashBySanta(int santa_num, int Mr, int Mc) {
    ...
    if (Rr + D * Mr<1 || Rr + D * Mr>N || Rc + D * Mc<1 || Rc + D * Mc>N) {
        santa_status[santa_num].first.first = 2; //탈락
    }
    else if (santa_matrix[Rr + D * Mr][Rc + D * Mc]) { //밀려날 곳:[Rr+D*Mr][Rc+D*Mc]에 다른 산타가 있다면 상호작용
