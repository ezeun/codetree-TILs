...
// 디버깅하려고 함수로 만듦
int find_direction(const int idx, const vector<int> directions)
{
    for (int j = 0; j < 4; j++) {
        int dir = priority[idx][player[idx].direction][j];
...
void move_player(int turn){
    ...
    for(int p=1; p<=m; p++){ //각 플레이어를 어디로 이동시킬지 저장만 해두기!!(아직 이동 x)
        ...
        vector<int> myOwn; //본인이 독점계약한 칸의 방향
    ...
    for(int p=m; p>0; --p) {
        ...
        player[p].c += dy[whereToGo[p]];
				...
				//오름차순으로 하면 큰 숫자로 덮어씌이기 때문에 내림차순으로 값 덮어씀
